...
- `industry_summary`：行业完整名称、交易日、数据时间、主力净流入额；行业双榜/A2合并请求最多10行，单侧榜仍由工作流限制为5行。
- `industry_daily_full`：单交易日全部Wind末级行业及主力净流入额；允许单向查询返回100行警告，但只能在与反向查询按行业全名合并后使用。
- `industry_stock`：Wind代码、简称、行业完整名称、交易日、数据时间、主力净流入额、涨跌幅和可选主力占比；批量阶段B最多30行，每个行业仍由工作流限制为3行。

行业关系键以A1返回的完整路径为准。批量B只允许逐字精确关联；逐行业作用域补查可返回目标末级名称，但只能由`scripts/industry_pipeline.py`在请求已显式限定唯一行业层级与名称时恢复为A1完整路径。父级、兄弟行业或不唯一的路径片段不得恢复。
